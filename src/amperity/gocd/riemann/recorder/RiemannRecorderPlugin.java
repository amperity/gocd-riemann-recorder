package amperity.gocd.riemann.recorder;
...
@Extension
public class RiemannRecorderPlugin implements GoPlugin {

    public static final Logger LOGGER = Logger.getLoggerFor(RiemannRecorderPlugin.class);
    ...
    public GoPluginIdentifier pluginIdentifier() {
        return new GoPluginIdentifier("notification", Arrays.asList("4.0"));
    }
    ...
    public void initializeGoApplicationAccessor(GoApplicationAccessor accessor) {
        ...
        try {
            require.invoke(Clojure.read("amperity.gocd.riemann.recorder.plugin"));
            this.handler = Clojure.var("amperity.gocd.riemann.recorder.plugin", "handler");
        } catch (Exception ex) {
        ...
        try {
            IFn init = Clojure.var("amperity.gocd.riemann.recorder.plugin", "initialize!");
            IFn logger = getLoggerFn();
            init.invoke(logger, this.accessor);
        } catch (Exception ex) {
            LOGGER.error("Failed to initialize plugin recorder", ex);
            throw ex;
        }
    }
    ...
    /**
     * Constructs a new logging function to inject into the recorder.
     */
